update_phi_gpbart <- function(x_train,
                              res_vec,
                              nu,
                              phi_vector_p,
                              tau,
        ...
        for(i in 1:length(phi_vector_p)){
                ...
                old_log_like <- Reduce("+",lapply(t_nodes, function(node){ node_loglikelihood_gpbart(node = node,res_vec = res_vec,
                                                                                          tau = tau,tau_mu = tau_mu,nu = nu,phi_vector = phi_vector_p)}))
                new_log_like <- Reduce("+",lapply(t_nodes, function(node){ node_loglikelihood_gpbart(node = node,res_vec = res_vec,
...
update_nu_gpbart <- function(x_train,
                              res_vec,
                              nu,
                              phi_vector_p,
                              tau,
...
update_g_node <- function(node,
        ...
        distance_sq_matrix_test_star <- distance_matrix(m1 = x_train,m2 = x_test,phi_vector = phi_vector_p)
        distance_sq_matrix_test_star_star <- symm_distance_matrix(m1 = x_test,phi_vector = phi_vector_p)
...
update_g_gpbart <- function(tree,
        ...
        # new g
        g_sample_train <- numeric()
        g_sample_test <- numeric()
        # Getting terminal nodes
        t_nodes <- get_terminals(tree)
        ...
        for(i in 1:length(t_nodes)){
               g_aux <- update_g_node(node = t_nodes[[i]],x_train = x_train,x_test = x_test,
                                      ...
                                      tau_mu = tau_mu,nu = nu,phi_vector = phi_vector_p)

               g_sample[t_nodes[[i]]$obs_train] <- g_aux$train_sample
        ...
        return(list(g_sample = g_sample, g_sample_test = g_sample_test))
}
