update_phi_gpbart <- function(tree,
        ...
        for(i in 1:length(phi_vector_p)){
                ...
                # prior_log <- ((stats::dgamma(x = phi_proposal,shape = 2,rate = 1,log = TRUE)+stats::dgamma(x = phi_proposal,shape = 20,rate = 1,log = TRUE))-(stats::dgamma(x = phi_vector_p[i],shape = 2,rate = 1,log = TRUE)+stats::dgamma(x = phi_vector_p[i],shape = 20,rate = 1,log = TRUE)))
                prior_log <- ((stats::dgamma(x = phi_proposal,shape = 5,rate = 1,log = TRUE)-(stats::dgamma(x = phi_vector_p[i],shape = 5,rate = 1,log = TRUE))))
